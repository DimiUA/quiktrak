...
function clearUserInfo() {
    ...
    var ModalNewApp = !localStorage.ModalNewApp ? '' : localStorage.ModalNewApp;
    ...
    if (ModalNewApp) {
        localStorage.ModalNewApp = ModalNewApp;
    }
...
function showNewAppModal() {
    ...
    var appLink = 'https://play.google.com/store/apps/details?id=com.quiktrak.pro';
    if (window.device) {
        var platform = device.platform.toLowerCase();
        switch (platform) {
            case "ios":
                appLink = 'https://apps.apple.com/us/app/quiktrak-pro/id1505742400?ls=1';
                break;
        }
    }
    ...
    var modal = App.modal({
        title: '<div class="custom-modal-logo-wrapper"><img class="custom-modal-logo" src="resources/images/logo.png" alt=""/></div>',
        text: '<div class="custom-modal-text">' + LANGUAGE.PROMPT_MSG075 + '</div>',
        afterText: '<div class="list-block no-hairlines modal-checkbox">' +
            '<ul>' +
            '<li>' +
            '<label class="label-checkbox item-content">' +
            '<input type="checkbox" name="checkbox-not-show-modal-new-app" value="">' +
            '<div class="item-media">' +
            '<i class="icon icon-form-checkbox"></i>' +
            '</div>' +
            '<div class="item-inner">' +
        ...
        buttons: [{
            text: LANGUAGE.COM_MSG39,
            onClick: function(parent) {
                var checkboxState = parent.find('input[name="checkbox-not-show-modal-new-app"]').is(":checked");
                if (checkboxState) {
                    localStorage.ModalNewApp = checkboxState;
                }
            }
        },
            {
                text: LANGUAGE.COM_MSG62,
                bold: true,
                onClick: function(parent) {
                    var checkboxState = parent.find('input[name="checkbox-not-show-modal-new-app"]').is(":checked");
                    if (checkboxState) {
                    ...
                    if (typeof navigator !== "undefined" && navigator.app) {
                        navigator.app.loadUrl(appLink, { openExternal: true });
                    } else {
...
function setAssetListPosInfo(listObj) {
    ...
    JSON1.requestPost(url, data, function(result) {
            ...
            setTimeout(function() {
                ...
                }else if(!localStorage.ModalNewApp){
                    showNewAppModal();
                }
